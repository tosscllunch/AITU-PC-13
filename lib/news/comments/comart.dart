...
class _CommentsState extends State<Comments> {
  ...
  newComment(connection, comment) async {
    await connection.execute(
        "INSERT INTO commNews (_articleID, _email, _message, _date) VALUES ('${comment[0]}', '${comment[1]}', '${comment[2]}', now()::timestamp(0))");
  }
  ...
  commentsAmount(connection, articleID) async {
    var result = await connection
        .query("SELECT COUNT(*) FROM commNews WHERE _articleID = $articleID;");
  ...
  querywholeRow(connection, articleID, i) async {
    var result = await connection.query(
        "SELECT * FROM (SELECT * FROM commNews WHERE _articleID = $articleID ORDER BY _id DESC LIMIT $i) AS foo ORDER BY _id ASC LIMIT 1");
