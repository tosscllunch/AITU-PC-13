...
import 'package:flutter_application_1/alarm/models/alarm_hive_storage.dart';
...
class AlarmModel extends ChangeNotifier {
  ...
  Future<void> _scheduleAlarm(AlarmDataModel alarm) async {
    ...
    if (alarm.weekdays.isEmpty & (alarm.period == 0)) {
      await flutterLocalNotificationsPlugin.zonedSchedule(
        alarm.id,
        'Будильник в ${fromTimeToString(alarm.time)}',
        '${alarm.alarmName}',
        TZDateTime.local(
          alarm.time.year,
          alarm.time.month,
      ...
      for (var weekday in alarm.weekdays) {
        ...
        print(alarm.time.day - alarm.time.weekday + weekday);
        print("------");
      }
    } else {
      for (var i = 0; i < alarm.howlong; i++) {
        await flutterLocalNotificationsPlugin.zonedSchedule(
          // acts as an id, for cancelling later
          alarm.id * 100 + i,
          'Будильник в ${fromTimeToString(alarm.time)}',
        ...
        print(alarm.time.day + (alarm.period * i));
        print("------");
      }
    }
  }
